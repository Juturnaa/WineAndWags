...
for (let i = 8; i <= 20; i++) {
  ...
  db.query(`INSERT INTO waw.convo SELECT nextval('waw.convo_id_seq'), ${7}, ${i} WHERE NOT EXISTS (SELECT id FROM waw.convo WHERE user1 in (${7}, ${i}) AND user2 in (${7}, ${i}))`);
}
// -----------MESSAGES FOR USER ID 7----------- //
for (let i = 8; i <= 12; i++) {
  // FIRST MESSAGE SENT FROM USER 7
  db.query(`INSERT INTO waw.message (id, sender_id, body, time_stamp, convo_id) VALUES (DEFAULT, ${7}, 'hiii cute dog', DEFAULT, (select id from waw.convo where user1 in (${7}, ${i}) and user2 in (${7}, ${i})))`);
}
